...
export default function HomePage() {
  ...
  // Initialize LocalStorage & Service Worker
  useEffect(() => {
    const stored = getStoredPreferences();
    setPreferences(stored);
    setIsLoaded(true);

    // If first visit, show the friendly selection dialog
  ...
  const handlePreferencesSaved = (batch: string, majorOrSection: string) => {
    setPreferences({ batch, majorOrSection, hasOnboarded: true });
  };

  // Helper for simulation times
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col selection:bg-cyan-500/30 selection:text-cyan-200">
      {/* Top Navbar */}
      <Navbar
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-5 sm:py-7 space-y-6 pb-24 md:pb-12">
        ...
        {isSimulationMode && (
          <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between text-xs font-mono text-amber-300 animate-in fade-in">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 font-bold transition-colors cursor-pointer"
            >
              লাইভ সময়ে ফিরুন
            </button>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
          <div>
            <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
              ...
              <span className="text-xs font-mono font-normal px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 text-zinc-400">
                Fall 2026
              </span>
            </h2>
          ...
          {/* Segmented View Switcher: Today vs Week */}
          <div className="flex items-center p-1 rounded-xl bg-zinc-900 border border-zinc-800 self-start sm:self-center">
            <button
              type="button"
        ...
        {/* Content Section: Today Timeline or Week View */}
        ...
        {/* Live Simulator Preview Dock (for testing different campus times) */}
        <div className="p-4 rounded-2xl border border-zinc-800/80 bg-zinc-950/60 backdrop-blur-sm space-y-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-[11px] font-mono text-zinc-300 transition-colors cursor-pointer"
            >
              ১ম পিরিয়ড (10:15 AM)
            </button>
            <button
        ...
        {/* Footer info */}
        <footer className="pt-6 border-t border-zinc-800/60 text-center text-xs font-mono text-zinc-400 space-y-2">
          <p>
            {INSTITUTION_INFO.name} • {INSTITUTION_INFO.subTitle}
          </p>
          <p className="text-[11px] text-zinc-500">
            CLASSR Routine Terminal • Zero Backend • Client Powered
          </p>
          <div className="pt-2">
      ...
      <nav
        ...
        className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-zinc-950/95 border-t border-zinc-800 backdrop-blur-xl px-2 py-2 flex items-center justify-around text-[10px] font-mono text-zinc-400"
      >
        <button
          type="button"
