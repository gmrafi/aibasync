...
export function LiveStatusCard({
  ...
  const getGreeting = () => {
    ...
    if (studentName && studentName.trim().length > 0) {
      return `${timeGreeting}, ${studentName.trim()} | AIBA Sync লাইভ স্ট্যাটাস`;
    }
    return `${timeGreeting} | AIBA Sync লাইভ স্ট্যাটাস`;
  };
    ...
    return (
      <div className="relative overflow-hidden rounded-3xl border border-rose-200 dark:border-rose-500/40 bg-white dark:bg-slate-900/90 shadow-xl shadow-rose-500/5 dark:shadow-rose-950/20 p-6 sm:p-7 text-slate-900 dark:text-slate-100 transition-colors">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-rose-50 dark:bg-rose-500/15 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30 uppercase tracking-wider">
                চলমান ক্লাস • {cls.isClub ? "Club Activity" : `Period ${cls.period}`}
              </span>
            </div>
          </div>

          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-mono text-slate-700 dark:text-slate-300 self-start sm:self-auto">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2.5 pt-1 text-xs sm:text-sm font-mono">
            {/* Time Slot */}
            <span className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-950 hover:bg-slate-100 dark:hover:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-violet-400 text-slate-700 dark:text-slate-300 transition-colors cursor-pointer"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-sky-50 dark:bg-sky-500/15 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-500/30 uppercase tracking-wider">
                পরবর্তী ক্লাস • {next.isClub ? "Club Activity" : `Period ${next.period}`}
              </span>
            </div>
          </div>

          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/30 text-xs font-mono text-sky-700 dark:text-sky-300 self-start sm:self-auto">
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-emerald-200 dark:border-emerald-500/30 bg-white dark:bg-slate-900/90 shadow-lg p-6 sm:p-7 text-slate-900 dark:text-slate-100 transition-colors">
      <div className="absolute top-0 left-0 w-2 h-full bg-emerald-500" />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        <div className="flex items-start gap-4">
          <div className="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-600 dark:text-emerald-400 mt-1 shadow-sm shrink-0">
          ...
          <div>
            ...
            <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
              আজকের নির্ধারিত সকল ক্লাস সম্পন্ন হয়েছে
            </h3>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
              পরবর্তী ক্লাসসূচি আগামী কর্মদিবসে যথাসময়ে কার্যকর হবে।
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>উপলব্ধ ক্লাসরুম নিরীক্ষণ</span>
        </button>
      </div>
    </div>
  );
